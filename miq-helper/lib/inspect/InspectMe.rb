...
class InspectMe
  ...
  def dump_root()
    @helper.log(:info, "Begin $evm.root.attributes")
    $evm.root.attributes.sort.each { |k, v| @helper.log(:info, "\t Attribute: #{k} = #{v}")}
    @helper.log(:info, "End $evm.root.attributes")
    @helper.log(:info, "")
  end

  alias :dumpRoot :dump_root

  def dump_attributes(object_type, object)
    @helper.log(:info, "Begin #{object_type}.attributes")
    if object.respond_to?('attributes')
      if $evm.root['user'].current_group.description == 'EvmGroup-super_administrator'
        @helper.log(:info, "\t #{object.name} - Attribute: 'authentication_userid' = #{object.authentication_userid.inspect}") if object.respond_to?('authentication_userid')
        @helper.log(:info, "\t #{object.name} - Attribute: 'authentication_password_encrypted' = #{object.authentication_password_encrypted.inspect}") if object.respond_to?('authentication_password_encrypted')
        @helper.log(:info, "\t #{object.name} - Attribute: 'authentication_password' = #{object.authentication_password.inspect}") if object.respond_to?('authentication_password')
      end
      object.attributes.sort.each { |k, v| @helper.log(:info, "\t #{object.name rescue object.description rescue nil} - Attribute: #{k.inspect} = #{v.inspect}") }
    elsif object.respond_to?('each')
      object.each { |obj| obj.attributes.sort.each { |k,v| @helper.log(:info, "\t #{obj.name rescue obj.description rescue nil} - Attribute: #{k.inspect} = #{v.inspect}")}}
    elsif object.respond_to?('count')
      @helper.log(:info, "\t #{object_type}: #{object.count}")
    elsif object_type == 'current_object'
      @helper.log(:info, "\t #{object.name} - Attribute: 'class_name' = #{object.class_name.inspect}")
      @helper.log(:info, "\t #{object.name} - Attribute: 'current_message' = #{object.current_message.inspect}")
      @helper.log(:info, "\t #{object.name} - Attribute: 'current_field_name' = #{object.current_field_name.inspect}")
      @helper.log(:info, "\t #{object.name} - Attribute: 'current_field_type' = #{object.current_field_type.inspect}")
      @helper.log(:info, "\t #{object.name} - Attribute: 'instance_name' = #{object.instance_name.inspect}")
      @helper.log(:info, "\t #{object.name} - Attribute: 'namespace' = #{object.namespace.inspect}")
      @helper.log(:info, "\t #{object.name} - Attribute: 'children' = #{object.children.inspect}")
    else
      @helper.log(:info, "\t #{object_type}: #{object.inspect}")
    end
    @helper.log(:info, "End #{object_type}.attributes")
    @helper.log(:info, "")
  end

  alias :dumpAttributes :dump_attributes

  def dump_associations(object_type, object)
    @helper.log(:info, "Begin #{object_type}.associations")
    if object.respond_to?('associations')
      object.associations.sort.each { |assc| @helper.log(:info, "\t #{object.name rescue object.description rescue nil} - Association: #{assc}") }
    else
      object.each {|obj| obj.associations.sort.each { |assc| @helper.log(:info, "\t #{object.name rescue object.description rescue nil} - Association: #{assc}") } }
    end
    @helper.log(:info, "End #{object_type}.associations")
    @helper.log(:info, "")
  end

  alias :dumpAssociations :dump_associations

  def dump_tags(object_type, object)
    @helper.log(:info, "Begin #{object_type}.tags")
    if object.respond_to?('tags')
      object.tags.sort.each do |tag_element|
        tag_text = tag_element.split('/')
        @helper.log(:info, "\t #{object.name rescue object.description rescue nil} - Category: #{tag_text.first.inspect} Tag: #{tag_text.last.inspect}")
      end
    else
      object.each do |obj|
        obj.tags.sort.each do |tag_element|
          tag_text = tag_element.split('/')
          @helper.log(:info, "\t #{obj.name rescue obj.description rescue nil} - Category: #{tag_text.first.inspect} Tag: #{tag_text.last.inspect}")
        end
      end
    end
    @helper.log(:info, "End #{object_type}.tags")
    @helper.log(:info, "")
  end

  alias :dumpTags :dump_tags

  def dump_virtual_columns(object_type, object)
    @helper.log(:info, "Begin #{object_type}.virtual_column_names")
    if object.respond_to?('virtual_column_names')
      object.virtual_column_names.sort.each { |vcn| @helper.log(:info, "\t #{object.name rescue object.description rescue nil} - Virtual Column: #{vcn} = #{object.send(vcn).inspect}") }
    else
      object.each {|obj| obj.virtual_column_names.sort.each { |vcn| @helper.log(:info, "\t #{obj.name rescue obj.description rescue nil} - Virtual Column: #{vcn} = #{obj.send(vcn).inspect}") } }
    end
    @helper.log(:info, "End #{object_type}.virtual_column_names")
    @helper.log(:info, "")
  end

  alias :dumpVirtualColumns :dump_virtual_columns
  ...
  alias :dumpCurrentObject :dump_current_object
  ...
  alias :dumpMIQServer :dump_miq_server
  alias :dumpMiqServer :dump_miq_server
  ...
  alias :dumpUser :dump_user

  # Group Information
  def dump_miq_group()
    object_type = 'miq_group'
  ...
  alias :dumpMIQGroup :dump_miq_group
  alias :dumpMiqGroup :dump_miq_group
  ...
  alias :dumpEmsCluster :dump_ems_cluster
  alias :dumpEMSCluster :dump_ems_cluster
  ...
  alias :dumpExtManagementSystem :dump_ext_management_system
  ...
  alias :dumpService :dump_service
  ...
  alias :dumpServiceTemplate :dump_service_template
  ...
  alias :dumpStorage :dump_storage

  # VM Information
  def dump_vm()
    object_type = $evm.root['vmdb_object_type']
    object = $evm.vmdb('vm').find_by_id($evm.root['vm_id'])
    dump_attributes(object_type, object)
    dump_associations(object_type, object)
    dump_tags(object_type, object)
    dump_virtual_columns(object_type, object)
