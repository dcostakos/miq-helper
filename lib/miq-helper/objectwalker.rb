...
class MiqHlp
  module ObjectWalker
    ...
    def walk_object()
      ...
      log(:info, "#{@method} #{VERSION} - EVM Automate Method Started")
      ...
      if @dump_methods
        ...
        unless user.nil?
          if user.method_missing(:class).superclass.name == "MiqAeMethodService::MiqAeServiceModelBase"
            ...
            log(:error, "#{@method} Unexpected parent class of $evm.root['user']: #{user.method_missing(:class).superclass.name}")
            @dump_methods = false
          end
        else
          log(:error, "#{@method} $evm.root['user'] doesn't exist")
          @dump_methods = false
        end
      end
      #
      ...
      #
      log(:info, "     #{@method}:   $evm.current_namespace = #{$evm.current_namespace}   #{type($evm.current_namespace)}")
      log(:info, "     #{@method}:   $evm.current_class = #{$evm.current_class}   #{type($evm.current_class)}")
      log(:info, "     #{@method}:   $evm.current_instance = #{$evm.current_instance}   #{type($evm.current_instance)}")
      log(:info, "     #{@method}:   $evm.current_message = #{$evm.current_message}   #{type($evm.current_message)}")
      log(:info, "     #{@method}:   $evm.current_object = #{$evm.current_object}   #{type($evm.current_object)}")
      log(:info, "     #{@method}:   $evm.current_object.current_field_name = #{$evm.current_object.current_field_name}   #{type($evm.current_object.current_field_name)}")
      log(:info, "     #{@method}:   $evm.current_object.current_field_type = #{$evm.current_object.current_field_type}   #{type($evm.current_object.current_field_type)}")
      log(:info, "     #{@method}:   $evm.current_method = #{$evm.current_method}   #{type($evm.current_method)}")
      log(:info, "     #{@method}:   $evm.root = #{$evm.root}   #{type($evm.root)}")
      #
      ...
      #
      log(:info, "     #{@method}:   $evm.parent = #{$evm.parent}   #{type($evm.parent)}")
